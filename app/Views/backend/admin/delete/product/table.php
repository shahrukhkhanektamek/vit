<table id="buttons-datatables1" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
    <thead>
        <tr>
            ...
            <th data-ordering="false">Model Year</th>
    ...
    <tbody>
        ...
        <?php foreach($data_list as $key=> $value) { ?>
            <tr>
                <td><img class="img-thumbnail" src="<?=image_check($value->images)?>" style="width:auto;height:50px;"></td>
                <td><?=$value->brand_name?></td> 
                <td><?=$value->category_name ?></td>
                <td><?=$value->name?></td> 
                <td><?=$value->model_year?></td>
                <td><?=price_formate($value->price) ?></td>
                <td><?=price_formate($value->booking_price) ?></td>
