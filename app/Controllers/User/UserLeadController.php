...
class UserLeadController extends BaseController
{
    ...
    public function load_data()
    {
        ...
        $query = $this->db->table($this->arr_values['table_name'])
            ...
            // ->join('users as partner', 'partner.id = ' . 'enquiry_lead.partner_id', 'left')
    ...
    public function scratch()
    {
        ...
        ->select("{$this->arr_values['table_name']}.*, 
            ...
            ")
        ->where([$this->arr_values['table_name'].".id"=>$id,$this->arr_values['table_name'].".user_id"=>$user_id,])->get()->getFirstRow();
        if(empty($row))
        {
            $action = 'reload';
            $responseCode = 200;
